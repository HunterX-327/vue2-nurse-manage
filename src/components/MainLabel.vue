    ...
    <div class="label">
        ...
        <div class="container" ref="container">
            <div class="main" ref="main" :style="mainStyle">
                <!-- home固定tag -->
                <el-tag @click="routeTo(tagHome)" size="medium" ref="home" :key="tagHome.name" :type="tagHome.type"
                    effect="dark">{{tagHome.name}}</el-tag>
                <el-tag @click="routeTo(tag)" @close="handleClose(tag)" size="medium" closable v-for="tag in tags"
                    :ref="`tag-${tag.path}`" :key="tag.name" :type="tag.type" effect="plain">{{tag.name}}</el-tag>
            </div>
        </div>
